...
use args::PangenomeSingleMergeArgs;
use clap::Parser;
use std::error::Error;
use std::fs::File;
...
bedtools single merge for the pangenome bed files. A tuple based approach
to anchor the first iter and then look for the next iter and then use the
comparison to anchor the merge.

*/

fn main() {
    let args = PangenomeSingleMergeArgs::parse();
    let output = pangenome_merge(&args.bed1, &args.fasta).unwrap();
    println!("Results have been written:{}", output);
}

fn pangenome_merge(path1: &str, path2: &str) -> Result<String, Box<dyn Error>> {
